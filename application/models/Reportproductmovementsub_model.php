...
/**
 * Description of reportproductmovementsub_model
 *
 * @Prasan Srisopa
 */
class Reportproductmovementsub_model extends CI_Model{
    ...
    //แม่เข้า 1
    public function in_product($product_id ,$date_start = null,$shop_id) {
    ...
    //แม่เข้า 2
    public function product_in($product_id ,$date_start = null,$date_end = null,$shop_id) {
        $this->db->select('SUM(receipt_detail.product_amount) AS product_amount');
        $this->db->join('receipt_detail', 'receipt_detail.receipt_master_id_pri = receipt_master.receipt_master_id_pri');
        $this->db->where('receipt_master.status_receipt_id', 1);
        $this->db->where('receipt_master.status_transfer_id', 3);
