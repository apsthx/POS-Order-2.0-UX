...
class Sell extends CI_Controller {
    ...
    public function index() {
        $data = array(
            ...
            'js' => array('flash-message.js', 'jquery-ui.js', 'build/sell.js'),
        );
        $this->renderView('sell_view', $data);
    }
    ...
    public function get_product_by_id() {
        ...
        $json = array();
        $json['error'] = "";
        $product_list = $this->sell_model->get_product($product_id);
    ...
    public function ajax_product_modal() {
        ...
        $data = array(
            ...
            'datas' => $this->sell_model->get_product_by_category($product_category_id)
        );
    ...
    public function save() {
        ...
        $check_receipt_master = $this->sell_model->check_receipt_master_id($receipt_master_id);
        ...
        if (sizeof($product_id_arr) > 0) {
            for ($i = 0; $i < sizeof($product_id_arr); $i++) {
                $sum_in_stock = $this->sell_model->sum_in_stock($product_id_arr[$i])->row()->sum;
        ...
        $bank = $this->sell_model->get_bank()->row();
        $data = array(
...
//        -- add log --
        ...
        $this->sell_model->bank_update($bank_update);
    ...
    private function delete_in_stock($product_id_arr, $product_amount_arr) {
        for ($i = 0; $i < sizeof($product_id_arr); $i++) {
            if ($product_id_arr[$i] != "" && $product_amount_arr[$i] != "") {
                ...
                $product_row = $this->sell_model->get_product($product_id_arr[$i])->row();
                $stocks = $this->sell_model->get_stock($product_id_arr[$i]);
                ...
                $this->sell_model->product_update($update_product);
                ...
                foreach ($stocks->result() as $stock) {
                    if ($stock->map_product_amount >= $amount_all) {
                        ...
                        $this->sell_model->map_stock_update($update);
                        break;
                    } else {
