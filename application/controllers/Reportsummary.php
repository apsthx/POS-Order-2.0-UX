...
class Reportsummary extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('reportsummary_model');
    ...
    public function index() {
        $data = array(
            ...
            'js' => array('build/reportsummary.js'),
        );
        $this->renderView('reportsummary_view', $data);
    }
    ...
    public function data() {
        ...
        if ($checked == 1) {
            $datas = $this->reportsummary_model->get_receipt_master_day($dateday);
        } else if ($checked == 2) {
            $datas = $this->reportsummary_model->get_receipt_master_month($datemonth);
        } else if ($checked == 3) {
            $datas = $this->reportsummary_model->get_receipt_master_year($dateyear);
        } else if ($checked == 4) {
            $datas = $this->reportsummary_model->get_receipt_master_dateday($datedaystart, $datedayend);
        } else {
            $datas = $this->reportsummary_model->get_receipt_master_all();
        }
        $data = array(
            'datas' => $datas,
            'customer_group_id' => $customer_group_id
        );
        $this->load->view('ajax/reportsummary_page', $data);
    }
...
//        $datedayend = $this->input->post('datedayend');
        ...
        $sheet->setTitle('รายงานสรุปยอดขาย');
        ...
        if ($checked == 1) {
            $datas = $this->reportsummary_model->get_receipt_master_day($dateday);
            $sheet->setCellValue("A1", 'รายงานสรุปยอดขายประจำวันที่ ' . $this->mics->dateen2stringthMS($dateday));
            $this->excel->getActiveSheet()->mergeCells("A1:G1");
        } else if ($checked == 2) {
            $datas = $this->reportsummary_model->get_receipt_master_month($datemonth);
            $sheet->setCellValue("A1", 'รายงานสรุปยอดขายประจำเดือน ' . $this->mics->date2thai("$datemonth-01", '%m %y'));
            $this->excel->getActiveSheet()->mergeCells("A1:G1");
        } else if ($checked == 3) {
            $datas = $this->reportsummary_model->get_receipt_master_year($dateyear);
            $sheet->setCellValue("A1", 'รายงานสรุปยอดขายประจำปี ' . ($dateyear + 543));
            $this->excel->getActiveSheet()->mergeCells("A1:G1");
        } else if ($checked == 4) {
            $datas = $this->reportsummary_model->get_receipt_master_dateday($datedaystart, $datedayend);
            $sheet->setCellValue("A1", 'รายงานสรุปยอดขายตั้งแต่ ' . $this->mics->dateen2stringthMS($datedaystart) . ' ถึง ' . $this->mics->dateen2stringthMS($datedayend));
            $this->excel->getActiveSheet()->mergeCells("A1:G1");
        } else {
            $datas = $this->reportsummary_model->get_receipt_master_all();
            $sheet->setCellValue("A1", 'รายงานสรุปยอดขายทั้งหมด');
            $this->excel->getActiveSheet()->mergeCells("A1:G1");
        }
        ...
        $filename = 'รายงานสรุปยอดขาย ข้อมูล ณ วันที่' . date('YmdHis') . '.xlsx';
