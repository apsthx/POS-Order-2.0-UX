...
/**
 * Description of Reportsummaryproduct
 *
 * @author Prasan Srisopa
 */
class Reportsummaryproduct extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('reportsummaryproduct_model');
    ...
    public function index() {
        $data = array(
            ...
            'js' => array('build/reportsummaryproduct.js'),
        );
        $this->renderView('reportsummaryproduct_view', $data);
    }
    ...
    public function data() {
        ...
        if ($checked == 1) {
            $datas = $this->reportsummaryproduct_model->get_receipt_master_day($dateday);
        } else if ($checked == 2) {
            $datas = $this->reportsummaryproduct_model->get_receipt_master_month($datemonth);
        } else if ($checked == 3) {
            $datas = $this->reportsummaryproduct_model->get_receipt_master_year($dateyear);
        } else if ($checked == 4) {
            $datas = $this->reportsummaryproduct_model->get_receipt_master_dateday($datedaystart, $datedayend);
        } else {
            $datas = $this->reportsummaryproduct_model->get_receipt_master_all();
        }
        $data = array(
            'datas' => $datas,
            'customer_group_id' => $customer_group_id
        );
        $this->load->view('ajax/reportsummaryproduct_page', $data);
    }
    ...
    public function export($checked, $dateday, $datemonth, $dateyear, $datedaystart, $datedayend, $customer_group_id) {
        ...
        $sheet->setCellValue("D2", 'สินค้า (รหัส)');
        $sheet->setCellValue("E2", 'จำนวน : หน่วย');
        ...
        if ($datas->num_rows() > 0) {
            foreach ($datas->result() as $data) {
                ...
                if ($customer_groups->num_rows() > 0) {
                    ...
                    $sheet->setCellValue("D$l", $data->product_name . ' (' . $data->product_id . ')');
                    $sheet->setCellValue("E$l", $data->product_amount . ' ' . $data->product_unit);
                    ...
                    $sheet->setCellValue("I$l", number_format($data->product_price_sum, 2));
                    ...
                    if ($data->type_receipt_id == 1) {
                        ...
                        $sheet->setCellValue("D$l", $data->product_name . ' (' . $data->product_id . ')');
                        $sheet->setCellValue("E$l", $data->product_amount . ' ' . $data->product_unit);
                        $sheet->setCellValue("F$l", 'ขายหน้าร้าน');
                        ...
                        $sheet->setCellValue("D$l", $data->product_name . ' (' . $data->product_id . ')');
                        $sheet->setCellValue("E$l", $data->product_amount . ' ' . $data->product_unit);
                        $sheet->setCellValue("F$l", 'สาขา/ตัวแทนจำหน่าย');
                        $sheet->setCellValue("G$l", $data->customer_name);
                        $sheet->setCellValue("H$l", $this->mics->dateen2stringthMS($data->date_pay));
                        $sheet->setCellValue("I$l", number_format($data->product_price_sum, 2));
                        $price_sum_pay += $data->product_price_sum;
