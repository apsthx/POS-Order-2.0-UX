...
/**
 * Description of Reportinvoice
 *
 * @author Prasan Srisopa
 */
class Reportinvoice extends CI_Controller{
    //put your code here
    public $group_id = 10;
    public $menu_id = 75;
    ...
    public function __construct() {
        ...
        $this->load->model('reportinvoice_model');
    ...
    public function index() {
        $data = array(
            ...
            'js' => array('build/reportinvoice.js'),
        );
        $this->renderView('reportinvoice_view', $data);
    }
    ...
    public function data() {
        ...
        if ($checked == 1) {
            $datas = $this->reportinvoice_model->get_receipt_master_day($dateday);
        } else if ($checked == 2) {
            $datas = $this->reportinvoice_model->get_receipt_master_month($datemonth);
        } else if ($checked == 3) {
            $datas = $this->reportinvoice_model->get_receipt_master_year($dateyear);
        } else if ($checked == 4) {
            $datas = $this->reportinvoice_model->get_receipt_master_dateday($datedaystart, $datedayend);
        } else {
            $datas = $this->reportinvoice_model->get_receipt_master_all();
        }
        $data = array(
            'datas' => $datas,
        );
        $this->load->view('ajax/reportinvoice_page', $data);
    }
    ...
    public function export($checked, $dateday, $datemonth, $dateyear, $datedaystart, $datedayend) {
        ...
        if ($datas->num_rows() > 0) {
            foreach ($datas->result() as $data) {
                ...
                $sheet->setCellValue("E$l", $this->reportinvoice_model->ref_status_pay($data->status_pay_id)->row()->status_pay_name);
                $sheet->setCellValue("F$l", $this->reportinvoice_model->ref_status_transfer($data->status_transfer_id)->row()->status_transfer_name);
                $sheet->setCellValue("G$l", $this->reportinvoice_model->ref_status_receipt_order($data->status_receipt_order_id)->row()->status_receipt_order_name);
